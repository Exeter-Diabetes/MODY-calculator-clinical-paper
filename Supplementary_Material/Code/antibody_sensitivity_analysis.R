...
thresholds_UNITED_t1d_gad_all_genes <- calculate_thresholds_diagnostics(dataset.UNITED_type1_gad_all_genes$M, 
                                                                        predictions_dataset.UNITED_type1_gad_all_genes_with_T$prob)

thresholds_UNITED_t1d_gad_ia2_all_genes <- calculate_thresholds_diagnostics(dataset.UNITED_type1_gad_ia2_all_genes$M, 
                                                                            predictions_dataset.UNITED_type1_gad_ia2_all_genes_with_T$prob)

thresholds_UNITED_t1d_gad_ia2_znt8_all_genes <- calculate_thresholds_diagnostics(dataset.UNITED_type1_gad_ia2_znt8_all_genes$M, 
                                                                                 predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T$prob)


### 5%

gad_5 <- thresholds_UNITED_t1d_gad_all_genes %>%
  filter(Thresholds == 0.05) %>% 
  arrange(Thresholds) %>% 
  head()

gad_ia2_5 <-thresholds_UNITED_t1d_gad_ia2_all_genes %>%
  filter(Thresholds == 0.05) %>% 
  arrange(Thresholds)  %>% 
  head()

gad_ia2_znt8_5 <- thresholds_UNITED_t1d_gad_ia2_znt8_all_genes %>%
  filter(Thresholds == 0.05) %>% 
  arrange(Thresholds)  %>% 
  head()

antis_5 <- rbind(gad_5, gad_ia2_5, gad_ia2_znt8_5)
write_xlsx(antis_5, "antibody_sensitivity_table.xlsx")
...
thresholds_UNITED_t1d_gad_ia2_znt8_all_genes %>%
  filter(Thresholds == 0.1) %>% 
  arrange(Thresholds)  %>% 
  head()
...
### GAD only 
roc_T1D_with_T_united_gad_all_genes <- calc_roc(dataset.UNITED_type1_gad_all_genes %>% 
                                                  mutate(M = ifelse(is.na(M), 0, M)) %>% 
                                                  select(M) %>% 
                                                  unlist(), predictions_dataset.UNITED_type1_gad_all_genes_with_T_full, 
                                                thinning = 1000)
...
plot_roc_T1D_with_T_united_gad_all_genes <- ggplot() +
  ...
  ## average predictions
  geom_path(
    data = pROC::roc(response = dataset.UNITED_type1_gad_all_genes %>% 
                       mutate(M = ifelse(is.na(M), 0, M)) %>% 
                       select(M) %>% 
                       unlist(), predictor = predictions_dataset.UNITED_type1_gad_all_genes_with_T$prob, 
                     levels = c(0,1), 
                     direction = "<") %>%
      magrittr::extract(c(2:3)) %>%
      as.data.frame(),
...
### GAD and IA2
roc_T1D_with_T_united_gad_ia2_all_genes <- calc_roc(dataset.UNITED_type1_gad_ia2_all_genes %>% 
                                                      mutate(M = ifelse(is.na(M), 0, M)) %>% 
                                                      select(M) %>% 
                                                      unlist(), predictions_dataset.UNITED_type1_gad_ia2_all_genes_with_T_full, 
                                                    thinning = 1000)
...
plot_roc_T1D_with_T_united_gad_ia2_all_genes <- ggplot() +
  ...
  ## average predictions
  geom_path(
    data = pROC::roc(response = dataset.UNITED_type1_gad_ia2_all_genes %>% 
                       mutate(M = ifelse(is.na(M), 0, M)) %>% 
                       select(M) %>% 
                       unlist(), predictor = predictions_dataset.UNITED_type1_gad_ia2_all_genes_with_T$prob, 
                     levels = c(0,1), 
                     direction = "<") %>%
      magrittr::extract(c(2:3)) %>%
      as.data.frame(),
    aes(x = 1-specificities, y= sensitivities), 
    colour = "black"
  ) +
  theme_bw() +
  scale_y_continuous("Sensitivity", labels = scales::percent) +
...
### All antibodies
roc_T1D_with_T_united_all_genes <- calc_roc(dataset.UNITED_type1_gad_ia2_znt8_all_genes %>% 
                                              mutate(M = ifelse(is.na(M), 0, M)) %>% 
                                              select(M) %>% 
                                              unlist(), predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T_full, 
                                            thinning = 1000)

# AUC ROC
auc_roc_T1D_with_T_united_all_genes <- unname(data.frame(prob = predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T$prob) %>%
                                                        cbind(Mody = dataset.UNITED_type1_gad_ia2_znt8_all_genes$M) %>%
                                                        mutate(Mody = ifelse(is.na(Mody), 0, Mody)) %>%
                                                        pROC::roc(response = Mody, predictor = prob) %>%
                                                        magrittr::extract(c(9)) %>%
                                                        unlist())
...
plot_roc_T1D_with_T_united_all_genes <- ggplot() +
  ...
  geom_path(
    ...
    aes(x = 1-specificities, y= sensitivities, group = iteration), 
    colour = "grey"
  ) +
  ## average predictions
  geom_path(
    data = pROC::roc(response = dataset.UNITED_type1_gad_ia2_znt8_all_genes %>% 
                       mutate(M = ifelse(is.na(M), 0, M)) %>% 
                       select(M) %>% 
                       unlist(), predictor = predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T$prob, 
                     levels = c(0,1), 
                     direction = "<") %>%
      magrittr::extract(c(2:3)) %>%
      as.data.frame(),
...
plot_antibody_boxplot <- patchwork::wrap_plots(
  ...
  dataset.UNITED_type1_gad_all_genes %>%
    ...
    scale_y_continuous("MODY probability", 
  ...
  # all antibodies
  dataset.UNITED_type1_gad_ia2_znt8_all_genes %>%
    mutate(
      M = ifelse(is.na(M), 0, M)
    ) %>%
    select(A, M) %>%
    cbind(prob = predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T$prob) %>%
    drop_na() %>%
    mutate(
      total = n(),
      M = ifelse(M == 0, "Non-MODY", "MODY")
    ...
    scale_y_continuous("MODY probability", 
                       ...
                       limits = c(0, 0.7)) +
    scale_alpha_manual(values = c(1, 0)) +
    ggtitle(paste0("GAD & IA2 & ZnT8 tested (n=", nrow(dataset.UNITED_type1_gad_ia2_znt8_all_genes %>% filter(!is.na(A))), ")")) +
    facet_wrap(~A_label) +
    theme_bw() +
    theme(
      legend.position = "none"
