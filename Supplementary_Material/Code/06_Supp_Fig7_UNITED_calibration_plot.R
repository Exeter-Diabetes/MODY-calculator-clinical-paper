...
grouping_values_UNITED_type1_with_T <- predictions_UNITED_type1_with_T$prob[which(!is.na(UNITED_type1$M))] %>% 
...
interim_dataset <- data.frame(
  prob = predictions_UNITED_type1_with_T$prob[which(!is.na(UNITED_type1$M))],
  M = UNITED_type1$M[which(!is.na(UNITED_type1$M))]
) %>%
  mutate(
    group = as.numeric(cut(prob, 
                           breaks = c(0, grouping_values_UNITED_type1_with_T, 1)))
  )
...
dataset_plot <- dataset_plot %>%
  ...
  rbind(
    data.frame(
      mean = mean(predictions_UNITED_type1_with_T$prob[which(is.na(UNITED_type1$M))]),
...
grouping_values_UNITED_type2 <- predictions_UNITED_type2$prob[which(!is.na(UNITED_type2$M))] %>% 
  #we then find the 20th and 80th quantile for these individuals
  quantile(probs = c(0.33, 0.66))


interim_dataset <- data.frame(
  prob = predictions_UNITED_type2$prob,
  M = UNITED_type2$M
) %>%
  mutate(
    group = as.numeric(cut(prob, 
                           breaks = c(0, grouping_values_UNITED_type2_new, 1)))
  )
...
plot_calibration_UNITED_type2 <- dataset_plot %>%
  ggplot(aes(x = mean, y = fit)) +
  geom_abline(aes(intercept = 0, slope = 1), linetype = "dashed") +
  geom_smooth(method = 'lm', formula = y ~ x,
              aes(fill = after_scale(color)), alpha = 0) +
...
pdf("Supplementary_Material/Outputs/SupFig9.pdf", width = 6, height = 9)
plot_calibration
dev.off()
