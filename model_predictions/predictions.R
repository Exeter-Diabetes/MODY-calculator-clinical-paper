...
dataset.case_control_type1 <- create_data(dataset = "case-control t1d", id = TRUE)
dataset.case_control_type2 <- create_data(dataset = "case-control t2d", id = TRUE)

## Load population representative dataset
dataset.UNITED_type1 <- create_data(dataset = "united t1d", id = TRUE)
dataset.UNITED_type2 <- create_data(dataset = "united t2d", id = TRUE)

## Load population representative dataset - all genes
dataset.UNITED_type1_genes <- create_data(dataset = "united t1d", commonmody = FALSE, id = TRUE)
dataset.UNITED_type2_genes <- create_data(dataset = "united t2d", commonmody = FALSE, id = TRUE)

## Load population representative dataset - pediatric cases
dataset.UNITED_type1_young_genes <- create_data(dataset = "united t1d pediatrics", commonmody = FALSE, id = TRUE)

## Load population representative dataset
dataset.UNITED_type1_gad <- create_data(dataset = "united t1d", biomarkers = "full", id = TRUE) %>%
...
dataset.UNITED_type1_gad_ia2 <- create_data(dataset = "united t1d", biomarkers = "full", id = TRUE) %>%
...
## Load population representative dataset all genes
dataset.UNITED_type1_gad_all_genes <- create_data(dataset = "united t1d", biomarkers = "full", commonmody = FALSE, id = TRUE) %>%
...
dataset.UNITED_type1_gad_ia2_all_genes <- create_data(dataset = "united t1d", biomarkers = "full", commonmody = FALSE, id = TRUE) %>%
...
dataset.UNITED_type1_gad_ia2_znt8_all_genes <- create_data(dataset = "united t1d", biomarkers = "full", commonmody = FALSE, id = TRUE) %>%
  
  # check if the antibody variable in question is recorded
  mutate(
    A = if_else(
      rowSums(!is.na(cbind(ZnT8, GAD, IA2))) == 0,  # all NA?
      NA,
      ifelse(rowSums(cbind(ZnT8, GAD, IA2) == 1, na.rm = TRUE) > 0, 1, 0)
    )
  ) %>%
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_genes, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_genes_sensitivity_analysis_with_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms)

colnames(predictions_dataset.UNITED_type1_genes_sensitivity_analysis_with_T_full) <- interim$id
...
### Predictions from new model with T
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_young_genes, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_young_all_genes_with_T_full <- predict(posterior_samples_T1D_all_genes_obj, select(interim, -id), rcs_parms_all_genes)

colnames(predictions_dataset.UNITED_type1_young_all_genes_with_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_young_genes %>% mutate(C = NA, A = NA), id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_young_all_genes_no_T_full <- predict(posterior_samples_T1D_all_genes_obj, select(interim, -id), rcs_parms_all_genes)

colnames(predictions_dataset.UNITED_type1_young_all_genes_no_T_full) <- interim$id

#### save the predictions
saveRDS(predictions_dataset.UNITED_type1_young_all_genes_no_T_full, "model_predictions/predictions_dataset.UNITED_type1_young_all_genes_no_T_full.rds")

predictions_dataset.UNITED_type1_young_all_genes_no_T <- predictions_dataset.UNITED_type1_young_all_genes_no_T_full %>%
  apply(., 2, function(x) {
    data.frame(prob = mean(x), LCI = quantile(x, probs = 0.025), UCI = quantile(x, probs = 0.975))
  }) %>%
  bind_rows() %>%
...
#### save the predictions
saveRDS(predictions_dataset.UNITED_type1_young_all_genes_no_T, "model_predictions/predictions_dataset.UNITED_type1_young_all_genes_no_T.rds")
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_young_genes, id, pardm, agerec, hba1c, agedx, sex)))

predictions_dataset.UNITED_type1_young_all_genes_old_full <- predict(posteriors_samples_old_T1D, select(interim, -id))

colnames(predictions_dataset.UNITED_type1_young_all_genes_old_full) <- interim$id
...
predictions_dataset.UNITED_type1_young_all_genes_old <- predictions_dataset.UNITED_type1_young_all_genes_old %>%
  cbind(id = interim$id)
...
predictions_dataset.UNITED_type1_all_genes_with_T_full <- predict(posterior_samples_T1D_all_genes_obj, select(interim, -id), rcs_parms_all_genes)

colnames(predictions_dataset.UNITED_type1_all_genes_with_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_genes %>%
                                        mutate(C = NA, A = NA), id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_all_genes_no_T_full <- predict(posterior_samples_T1D_all_genes_obj, select(interim, -id), rcs_parms_all_genes)

colnames(predictions_dataset.UNITED_type1_all_genes_no_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_genes, id, pardm, agerec, hba1c, agedx, sex)))

predictions_dataset.UNITED_type1_all_genes_old_full <- predict(posteriors_samples_old_T1D, select(interim, -id)) 

colnames(predictions_dataset.UNITED_type1_all_genes_old_full) <- interim$id
...
predictions_dataset.UNITED_type1_all_genes_old <- predictions_dataset.UNITED_type1_all_genes_old %>%
  cbind(id = interim$id)
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type2_genes, id, pardm, agerec, hba1c, agedx, sex, bmi, insoroha)))

predictions_dataset.UNITED_type2_genes_sensitivity_analysis_new_full <- predict(posterior_samples_T2D_obj, select(interim, -id)) 

colnames(predictions_dataset.UNITED_type2_genes_sensitivity_analysis_new_full) <- interim$id
...
predictions_dataset.UNITED_type2_all_genes_new_full <- predict(posterior_samples_T2D_all_genes_obj, select(interim, -id)) 

colnames(predictions_dataset.UNITED_type2_all_genes_new_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type2_genes, id, agedx, bmi, hba1c, pardm, agerec, insoroha, sex)))

predictions_dataset.UNITED_type2_all_genes_old_full <- predict(posteriors_samples_old_T2D, select(interim, -id)) 

colnames(predictions_dataset.UNITED_type2_all_genes_old_full) <- interim$id
...
predictions_dataset.UNITED_type2_all_genes_old <- predictions_dataset.UNITED_type2_all_genes_old %>%
  cbind(id = interim$id)
...
#UNITED all antibody
### Predictions from new model with T
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_sensitivity_analysis_with_T_full <- predict(posterior_samples_T1D_sensivity_analysis_obj, select(interim, -id), rcs_parms)

colnames(predictions_dataset.UNITED_type1_sensitivity_analysis_with_T_full) <- interim$id
...
# UNITED gad dataset
### Predictions from new model with T
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_gad, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_gad_sensitivity_analysis_with_T_full <- predict(posterior_samples_T1D_sensivity_analysis_obj, select(interim, -id), rcs_parms)

colnames(predictions_dataset.UNITED_type1_gad_sensitivity_analysis_with_T_full) <- interim$id
...
predictions_dataset.case_control_type1_no_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms)

colnames(predictions_dataset.case_control_type1_no_T_full) <- interim$id
...
predictions_dataset.referral_type1_no_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms)

colnames(predictions_dataset.referral_type1_no_T_full) <- interim$id
...
predictions_dataset.UNITED_type1_no_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms) 

colnames(predictions_dataset.UNITED_type1_no_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.referral_type1, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A))) %>%
  mutate(across(-id, as.numeric))

predictions_dataset.referral_type1_with_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms)

colnames(predictions_dataset.referral_type1_with_T_full) <- interim$id
...
predictions_dataset.UNITED_type1_with_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms) 

colnames(predictions_dataset.UNITED_type1_with_T_full) <- interim$id
...
predictions_dataset.UNITED_type1_gad_with_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms) 

colnames(predictions_dataset.UNITED_type1_gad_with_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_gad_all_genes, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_gad_all_genes_with_T_full <- predict(posterior_samples_T1D_all_genes_obj, select(interim, -id), rcs_parms_all_genes) 

colnames(predictions_dataset.UNITED_type1_gad_all_genes_with_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_gad_ia2, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_gad_ia2_with_T_full <- predict(posterior_samples_T1D_obj, select(interim, -id), rcs_parms) 

colnames(predictions_dataset.UNITED_type1_gad_ia2_with_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_gad_ia2_all_genes, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_gad_ia2_all_genes_with_T_full <- predict(posterior_samples_T1D_all_genes_obj, select(interim, -id), rcs_parms_all_genes) 

colnames(predictions_dataset.UNITED_type1_gad_ia2_all_genes_with_T_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1_gad_ia2_znt8_all_genes, id, pardm, agerec, hba1c, agedx, sex, bmi, C, A)))

predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T_full <- predict(posterior_samples_T1D_all_genes_obj, select(interim, -id), rcs_parms_all_genes) 

colnames(predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T_full) <- interim$id

#### save the predictions
saveRDS(predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T_full, "model_predictions/predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T_full.rds")

predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T <- predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T_full %>%
  apply(., 2, function(x) {
    data.frame(prob = mean(x), LCI = quantile(x, probs = 0.025), UCI = quantile(x, probs = 0.975))
  }) %>%
  bind_rows() %>%
...
#### save the predictions
saveRDS(predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T, "model_predictions/predictions_dataset.UNITED_type1_gad_ia2_znt8_all_genes_with_T.rds")
...
interim <- as_tibble(as.matrix(select(dataset.case_control_type1, id, pardm, agerec, hba1c, agedx, sex))) %>%
  mutate(across(-id, as.numeric))

predictions_dataset.case_control_type1_old_full <- predict(posteriors_samples_old_T1D, select(interim, -id)) 

colnames(predictions_dataset.case_control_type1_old_full) <- interim$id
...
predictions_dataset.case_control_type1_old <- predictions_dataset.case_control_type1_old %>%
  cbind(id = interim$id)
...
interim <- as_tibble(as.matrix(select(dataset.referral_type1, id, pardm, agerec, hba1c, agedx, sex))) %>%
  mutate(across(-id, as.numeric))

predictions_dataset.referral_type1_old_full <- predict(posteriors_samples_old_T1D, select(interim, -id)) 

colnames(predictions_dataset.referral_type1_old_full) <- interim$id
...
predictions_dataset.referral_type1_old <- predictions_dataset.referral_type1_old %>%
  cbind(id = interim$id)
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type1, id, pardm, agerec, hba1c, agedx, sex)))

predictions_dataset.UNITED_type1_old_full <- predict(posteriors_samples_old_T1D, select(interim, -id)) 

colnames(predictions_dataset.UNITED_type1_old_full) <- interim$id
...
predictions_dataset.UNITED_type1_old <- predictions_dataset.UNITED_type1_old %>%
  cbind(id = interim$id)
...
interim <- as_tibble(as.matrix(select(dataset.case_control_type2, id, pardm, agerec, hba1c, agedx, sex, bmi, insoroha))) %>%
  mutate(across(-id, as.numeric))

predictions_dataset.case_control_type2_new_full <- predict(posterior_samples_T2D_obj, select(interim, -id)) 

colnames(predictions_dataset.case_control_type2_new_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.referral_type2, id, pardm, agerec, hba1c, agedx, sex, bmi, insoroha))) %>%
  mutate(across(-id, as.numeric))

predictions_dataset.referral_type2_new_full <- predict(posterior_samples_T2D_obj, select(interim, -id)) 

colnames(predictions_dataset.referral_type2_new_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type2, id, pardm, agerec, hba1c, agedx, sex, bmi, insoroha)))

predictions_dataset.UNITED_type2_new_full <- predict(posterior_samples_T2D_obj, select(interim, -id))

colnames(predictions_dataset.UNITED_type2_new_full) <- interim$id
...
interim <- as_tibble(as.matrix(select(dataset.case_control_type2, id, agedx, bmi, hba1c, pardm, agerec, insoroha, sex))) %>%
  mutate(across(-id, as.numeric))

predictions_dataset.case_control_type2_old_full <- predict(posteriors_samples_old_T2D, select(interim, -id)) 

colnames(predictions_dataset.case_control_type2_old_full) <- interim$id
...
predictions_dataset.case_control_type2_old <- predictions_dataset.case_control_type2_old %>%
  cbind(id = interim$id)
...
interim <- as_tibble(as.matrix(select(dataset.referral_type2, id, agedx, bmi, hba1c, pardm, agerec, insoroha, sex))) %>%
  mutate(across(-id, as.numeric))

predictions_dataset.referral_type2_old_full <- predict(posteriors_samples_old_T2D, select(interim, -id)) 

colnames(predictions_dataset.referral_type2_old_full) <- interim$id
...
predictions_dataset.referral_type2_old <- predictions_dataset.referral_type2_old %>%
  cbind(id = interim$id)
...
interim <- as_tibble(as.matrix(select(dataset.UNITED_type2, id, agedx, bmi, hba1c, pardm, agerec, insoroha, sex)))

predictions_dataset.UNITED_type2_old_full <- predict(posteriors_samples_old_T2D, select(interim, -id)) 

colnames(predictions_dataset.UNITED_type2_old_full) <- interim$id
...
predictions_dataset.UNITED_type2_old <- predictions_dataset.UNITED_type2_old %>%
  cbind(id = interim$id)
